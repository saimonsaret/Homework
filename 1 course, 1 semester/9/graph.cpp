...
int findSpanningTree(int **edges, int nodes, int *firstNodes, int *secondNodes) {
	...
	memset(used, 0, nodes * sizeof(bool));
	...
	memset(from, -1, nodes * sizeof(int));
	...
	while (currentNode != -1) {
		step++;
		used[currentNode] = true;
		minEdge[currentNode] = maxInt;

		for (int i = 0; i < nodes; i++)
			if (edges[currentNode][i] != 0 && edges[currentNode][i] < minEdge[i] && !used[i]) {
				minEdge[i] = edges[currentNode][i];
				from[i] = currentNode;
			}
		...
		if (dest != -1) {
			firstNodes[step] = from[dest];
			secondNodes[step] = dest;
			res += min;
		}
		currentNode = dest;
	}
